...
import { Activity, AlertCircle, CheckCircle, Clock, RefreshCw, Share2, Settings, Loader2, Camera } from 'lucide-react'
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Please select a space from the dropdown above to view dashboard statistics.
            </p>
          </CardContent>
        </Card>
      </PageLayout>
    ...
    >
      ...
      {isCapturing && (
        <ThinStatusBar 
          ...
          message={`Capturing ${activeOperation?.type === 'bulk' ? 'multiple' : 'single'} snapshot(s)...`}
        />
      )}
          ...
          {stats.map((stat) => {
            ...
            return (
              <StatCard
                ...
                className={isClickable ? 'cursor-pointer hover:shadow-md transition-shadow' : undefined}
                ...
                {snapshots.slice(0, 5).map((snapshot: any) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-2 h-2 rounded-full ${
                          snapshot.status === 'success' ? 'bg-green-500' : 'bg-red-500'
                        }`}
                      />
                      <div>
                        <p className="font-medium">{snapshot.endpoint || snapshot.method}</p>
                        <p className="text-sm text-muted-foreground">
                          {formatRelativeTime(snapshot.timestamp || snapshot.created_at)}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className={`text-xs ${
                        snapshot.status === 'success' ? 'text-green-600' : 'text-red-600'
                      }`}>
